		...
		for( int idx = 0; idx < resultList.size(); idx++ ) {
			...
			for( HashMap result : resultList ) {
				...
				if( tmpServerSeq.equals( result.get( "SERVER_SEQ" ).toString())  ) {
					...
					/*
					 * System.out.println("test1:" + tmpServerSeq); System.out.println("test2:"
					 * +result.get( "SERVER_SEQ" ).toString()); System.out.println("test3:"
					 * +returnMap.get("SERVER_SEQ").toString());
					 */
					
					/*
					if( !returnMap.containsValue( result.get( "SERVER_SEQ" ).toString() ) ) {
					...
					*/
					...
					/*
					 * returnMap.put( result.get( "PROCESS_NAME" ).toString() + "MEMORY_USAGE",
					 * result.get( "MEMORY_USAGE" ).toString() );
					 */
					}
//					returnMap.put( "SERVER_SEQ", result.get( "SERVER_SEQ" ).toString() );
				}
